...
#Changes made to the commands used to produce added variable plots in line with changes made to the package "Car" in Version 2.0-9. 
#The "avp" command has been updated and become the "avPlot" command.

#Updated March 10, 2011
#Changes made to the commands used to produce transformations in line with changes made to the package "alr3" in Version 2.0.0. 
#The "bctrans" command has been updated and become the "powerTransform" command.

#Please change the file path in the command below to coincide with where you have stored the data files
setwd("C:/Users/sheather/Documents/docs/AModernApproachToRegression/Data")
...
m1 <- lm(Price~Food+Decor+Service+East)
par(mfrow=c(2,2))
avPlot(m1,variable="Food",ask=FALSE,id.method="identify")
# Click on the points you wish to identify. When you wish
# to stop click the right mouse button and select "Stop"
avPlot(m1,variable="Decor",ask=FALSE)
avPlot(m1,variable="Service",ask=FALSE)
avPlot(m1,variable="East",ask=FALSE)
...
avPlot(m1,variable="Tabloid.with.a.Serious.Competitor",ask=FALSE)
...
library(car)
par(mfrow=c(2,3))
avPlot(m1,variable="log(DArea)",ask=FALSE)
avPlot(m1,variable="log(CCost)",ask=FALSE)
avPlot(m1,variable="log(Dwgs)",ask=FALSE)
avPlot(m1,variable="log(Length)",ask=FALSE)
avPlot(m1,variable="log(Spans)",ask=FALSE)
...
library(car)
par(mfrow=c(2,4))
avPlot(m1,variable="log(ParkerPoints)",ask=FALSE,id.method="identify")
avPlot(m1,variable="log(CoatesPoints)",ask=FALSE,id.method="identify")
avPlot(m1,variable="P95andAbove",ask=FALSE)
avPlot(m1,variable="FirstGrowth",ask=FALSE)
avPlot(m1,variable="CultWine",ask=FALSE)
avPlot(m1,variable="Pomerol",ask=FALSE)
avPlot(m1,variable="VintageSuperstar",ask=FALSE)
...
summary(powerTransform(cbind(EngineSize,Cylinders,Horsepower,HighwayMPG,Weight,WheelBase)~1,data=cars04))
